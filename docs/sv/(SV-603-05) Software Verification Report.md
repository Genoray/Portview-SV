...
| Reference | Use |
| --- | --- |
| `SV-603-01` [Software Validation Report]((SV-603-01) Software Validation Report.md) | Validation basis and release conclusion |
| `SV-603-02` [Software Development Planning]((SV-603-02) Software Development Planning.md) | Lifecycle and configuration-management context |
| `SV-603-03` [Software High Level Design]((SV-603-03) Software High Level Design.md) | Architecture and decomposition reference |
| `SV-603-04` [Software Verification Plan]((SV-603-04) Software Verification Plan.md) | Governing plan |
| `RS-603` [RS for Portview]((RS-603) RS.md) | Integration verification input |
| `SRS-603` [SwSRS for Portview]((SRS-603) SwSRS.md) | Unit verification input |
| `SDS-603` [SwSDS for Portview]((SDS-603) SwSDS.md) | Software design reference |
| `STP-603` [SwSTP for Portview]((STP-603) SwSTP.md) | Unit verification procedure |
| `STR-603` [SwSTR-Z01 for Portview]((STR-603) SwSTR.md) | Unit verification record |
| `TP-603` [SwTP for Portview]((TP-603) SwTP.md) | Integration verification procedure |
| `TM-603` [Traceability Matrix]((TM-603) Traceability Matrix.md) | Requirement and design traceability reference |
| `TR-603` [SwTR-Z01 for Portview]((TR-603) SwTR.md) | Integration verification record |
| `SystemTR-603` [SystemTR-Z01]((SystemTR-603) SystemTR.md) | System verification result record |
...
| --- | --- | --- | --- | --- |
| Unit | Portview unit verification procedures | Executed | `Pass summary stated` | Unit execution is identified as `STR-603` |
| Integration | Portview integration verification procedures | Executed | `Pass summary stated` | Integration execution is identified as `TR-603` |
| System | Portview system-level verification procedures | Executed | `Pass summary stated` | System execution is summarized in `SystemTR-603` |
...
| Verification plan reference | `SV-603-04` version 1.6 with in-body reference to Rev 1.5 | Version identifiers should be aligned across the final document set |
| Software requirement baseline | `SRS-603` SwSRS for Portview Rev 1.6 | Unit verification source |
| Integration requirement baseline | `RS-603` RS for Portview Rev 0.1 | Integration verification source |
...
| --- | --- | --- |
| Unit verification environment | `Genoray testing platform / Default configuration` | Recorded in `STP-603` SwSTP for Portview |
| Integration verification environment | `Genoray testing platform / Default configuration` | Recorded in `TP-603` SwTP for Portview |
| System verification environment | `Controlled by SystemTR-603` | Separate environment details are not present in the currently available record set |
...
| --- | --- | --- | --- | --- |
| `STP-603` | [SwSTP for Portview]((STP-603) SwSTP.md) | `2025.07.24` | `J.W. Lee` | `Procedure baseline` |
| `STR-603` | [SwSTR-Z01 for Portview]((STR-603) SwSTR.md) | `2025.07.24` | `J.W. Lee` | `Pass summary claimed` |
| `TR-603` | [SwTR-Z01 for Portview]((TR-603) SwTR.md) | `2025.07.24` | `J.W. Lee` | `Pass summary claimed` |
| `SystemTR-603` | [SystemTR-Z01]((SystemTR-603) SystemTR.md) | `2026.03.18` | `J.W. Lee` | `Passed` |
...
- the governing unit procedure is `STP-603` [SwSTP for Portview]((STP-603) SwSTP.md) and the unit result record is `STR-603` [SwSTR-Z01 for Portview]((STR-603) SwSTR.md)
...
- integration verification was reported against `RS-603` RS for Portview software requirements
- the report references communication-function-related evaluation during integration testing
- the controlling integration procedure is `TP-603` [SwTP for Portview]((TP-603) SwTP.md) and the result record is `TR-603` [SwTR-Z01 for Portview]((TR-603) SwTR.md)

### 7.3 System Verification Results

System testing was conducted with `SystemTR-603` [SystemTR-Z01]((SystemTR-603) SystemTR.md), and system-level functions properly support the system specification.
...
- `Conditional:` Add the system-level execution date, executor, and controlled environment if they are maintained outside the currently available `SystemTR-603` record set.
